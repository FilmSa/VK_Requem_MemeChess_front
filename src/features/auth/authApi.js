...
function buildError(error, fallbackMessage) {
  ...
  if (rawMessage.includes("username must be 3-32")) {
    return new ApiError(
      "Имя пользователя должно содержать от 3 до 32 символов: буквы, цифры или подчёркивание.",
      {
        status: error.status,
        fields: {
          username: "От 3 до 32 символов: буквы, цифры или подчёркивание.",
        },
        payload: error.payload,
      }
    );
  ...
  if (
    ...
    return new ApiError(
      "Такой логин или почта уже используются. Попробуйте другие данные или войдите в существующий аккаунт.",
      {
        status: error.status,
        fields: {
